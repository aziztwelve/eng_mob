...
export function ListeningShadowingStep({ step, onSubmit, onContinue, isLast }: StepComponentProps) {
  ...
  if (!content) {
    return <View className="flex-1 items-center justify-center p-6"><Text className="text-muted-foreground">Не удалось распарсить content.</Text></View>;
  }
    ...
    try {
      ...
      if (accuracy < content.min_accuracy) {
        setState({
          ...
          explanation: `Точность ${Math.round(accuracy * 100)}%. Повторите фразу ещё раз: нужно не менее ${Math.round(content.min_accuracy * 100)}%.`,
        });
        return;
      }
  ...
  return (
    <View className="flex-1">
      <ScrollView className="flex-1 px-4 pt-4" contentContainerClassName="gap-4 pb-4">
        <Text className="text-foreground text-base font-bold">Слушайте фразу, затем повторите её вслух.</Text>
        <View className="rounded-2xl border border-white/20 bg-white/10 p-5">
          ...
          >
            ...
            <Text className="mt-2 font-black text-[#3D0A1A]">Слушать фразу</Text>
          </Pressable>
